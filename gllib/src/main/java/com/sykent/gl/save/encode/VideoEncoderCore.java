...
package com.sykent.gl.save.encode;
...
import com.sykent.gl.save.gles.L;
...
public class VideoEncoderCore {
    ...
    public static boolean checkConfig(EncodeConfig config) {
        boolean success;
        ...
        try {
            MediaCodec encoder = MediaCodec.createEncoderByType(config.mimeType);
...
//		}
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (mBufferInfo.size != 0) {
                    ...
                    if (mBufferInfo.presentationTimeUs < lastPresentationTimeUs) {
                        //fix flyme
                        mBufferInfo.presentationTimeUs = lastPresentationTimeUs;
                    }
    ...
    public static class EncodeConfig {
        ...
        public static final int I_FRAME_INTERVAL = 1;           // 1 seconds between I-frames
        ...
        public EncodeConfig(int width, int height, int frameRate, String outputPath) {
            this(width, height, frameRate, outputPath, I_FRAME_INTERVAL);
        }

        public EncodeConfig(int width, int height, int frameRate, String outputPath, int iFrameInterval) {
            this(width, height, frameRate, outputPath, iFrameInterval, 0.25f);
        }

        public EncodeConfig(int width, int height, int frameRate, String outputPath, int iFrameInterval, float compressionRatio) {
            this.width = width;
            this.height = height;
            this.outputPath = outputPath;
            this.iFrameInterval = iFrameInterval > 0 ? iFrameInterval : I_FRAME_INTERVAL;
            this.frameRate = frameRate <= 0 ? FRAME_RATE : frameRate;
            ...
            bitRate = (int) (compressionRatio * frameRate * width * height);
        }
